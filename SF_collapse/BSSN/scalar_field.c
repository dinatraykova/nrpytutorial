...
const REAL t_final =  600.; /* Final time is set so that at t=t_final, 
                            * data at the origin have not been corrupted 
                            * by the approximate outer boundary condition */
const REAL CFL_FACTOR = 0.5; /* Set the CFL Factor; 
                               * 0.5 sufficient for non-collapsing cases; 
                               * 0.25 for collapse to resolve oscillations*/
...
const REAL br_on = 1.0; // Turn on(1.0)/off(0.0) scalar field backreaction on the metric

// Choose quadratic or self-interaction potential; only one on at the same time
const REAL pot1_on = 1.0; // Turn on(1.0)/off(0.0) quadratic potential
const REAL pot2_on = 0.0; // Turn on(1.0)/off(0.0) self-interactiong potential

// Variables for the scalar field potential
const REAL scalarmass = 1.0; // Scalar mass, \mu = c/\hbar m M
REAL fa = 0.05; // Decay constant, only relevant for self-interacting potential; 0.05 for collapse
...
//Declare initial data arrays
FILE *uu_file = fopen("InitialData/phi001.csv", "r");
FILE *vv_file = fopen("InitialData/Pi001.csv", "r");
FILE *psi_file = fopen("InitialData/psi001.csv", "r");
FILE *alpha_file = fopen("InitialData/alpha001.csv", "r");
...
// Declare splines to interpolate onto the cartesian grid
...
int main(int argc, const char *argv[]) {
  ...
  // Check potentials; specify filemanes depending on type of potential
  char* pot;
  char decay[15];
  if (pot1_on == 1.0 && pot2_on == 0.0) {
      pot = "quad-pot";
      sprintf(decay, "");
      printf("Running quadratic potential, with scalar mass = %.2f\n",scalarmass);
  }
  if (pot1_on == 0.0 && pot2_on == 1.0) {
      pot = "selfint-pot";
      sprintf(decay, "_fa%.3f", fa);      
      printf("Running self-interacting potential, with scalar mass = %.2f and decay constant = %.2f\n",scalarmass,fa);
  }
  if (pot1_on != 0.0 && pot2_on != 0.0) {
    printf("Error: Only one potential should be on at the same time\n");
    pot = "";
    sprintf(decay, "");
    exit(1);
  }
  if (pot1_on == 0.0 && pot2_on == 0.0) {
      pot = "no-pot";
      sprintf(decay, "");
      printf("Running massless scalar.\n");
  }
  if (pot1_on != 0.0 && pot1_on != 1.0) {
      pot = "";
      sprintf(decay, "");
      printf("Error: pot1_on can only take values 0.0 or 1.0!\n");
      exit(1);
  }
    if (pot2_on != 0.0 && pot2_on != 1.0) {
      pot = "";
      sprintf(decay, "");
      printf("Error: pot2_on can only take values 0.0 or 1.0!\n");
      exit(1);
  }
    if (br_on != 0.0 && br_on != 1.0) {
      pot = "";
      sprintf(decay, "");
      printf("Error: br_on can only take values 0.0 or 1.0!\n");
      exit(1);
  }
...
#include "xxminmax.h"
  ...
  char filename2[100];  
  sprintf(filename2,"evolution/%s%s_uu_vv_cf.txt",pot,decay);
...
#pragma omp parallel for
    ...
    if(n%50 == 0) {
       // Evaluate Hamiltonian constraint violation
        Hamiltonian_constraint(Nxx,Nxx_plus_2NGHOSTS,dxx, xx, evol_gfs, aux_gfs);
        char filename[100];
        sprintf(filename,"output2D/%s%s_2d_t-%08d.txt",pot,decay,n);
