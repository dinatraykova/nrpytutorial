...
mSD = par.Cparameters("REAL", thismodule, "mSD") # Switch off source terms

br_on = par.Cparameters("REAL", thismodule, "br_on") # Switch on/off back-reaction
pot1_on = par.Cparameters("REAL", thismodule, "pot1_on") # Switch on/off quadratic potential
pot2_on = par.Cparameters("REAL", thismodule, "pot2_on") # Switch on/off self-interacting potential
scalarmass = par.Cparameters("REAL", thismodule, "scalarmass") # scalar mass
fa = par.Cparameters("REAL", thismodule, "fa") # axion decay constant
   ...
   # Step 16a: Define physical metric 
    ...
    for i in range(DIM):
        for j in range(DIM):
            ...
            Abar_rhsDD[i][j] += -8*PI*alpha * exp_m4phi * S_TFDD[i][j]

    # Step 18b: \partial_t K 
    trK_rhs += 4*PI*alpha*(rho + S)

    # Step 18c: Lambar_RHS
    for i in range(DIM):
        for j in range(DIM):
            Lambar_rhsU[i] += -16*PI*alpha*gammabarUU[i][j]*SD[j]
