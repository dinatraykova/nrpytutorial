...
const REAL br_on = 1.0; // Turn on(1.)/off(0.) scalar field backreaction on the metric
const REAL pot1_on = 1.0; // Turn on(1.)/off(0.) quadratic potential
const REAL pot2_on = 0.0; // Turn on(1.)/off(0.) self-interactiong potential
// Make sure at most one potential is on at a time
// Variables for the scalar field potential
const REAL scalarmass = 1.; // Scalar mass, \mu = c/\hbar m
const REAL fa = 0.5; // Decay constant
...
FILE *uu_file = fopen("InitialData/Pi001.csv", "r");
...
#pragma omp parallel for
  LOOP_REGION(0,Nxx_plus_2NGHOSTS[0], 0,Nxx_plus_2NGHOSTS[1], 0,Nxx_plus_2NGHOSTS[2]) {
    ...
    REAL xx2 = xCart[2];
    {
        ...
        in_gfs[IDX4(UUGF, i0, i1, i2)] = 0.0; //gsl_spline_eval (spline_u, r_scalar, acc);
...
#include "xxminmax.h"
  ...
  char filename2[100];
  sprintf(filename2,"evolution/quad_pot_uu_vv_cf.txt");
...
#pragma omp parallel for
    ...
    if(n%50 == 0) {
       ...
       // Hamiltonian_constraint(Nxx,Nxx_plus_2NGHOSTS,dxx, xx, evol_gfs, aux_gfs);
        char filename[100];
        sprintf(filename,"output2D/quad_pot_2d_t-%08d.txt",n);
